...
The application relies on a decoupled architecture, ensuring client-side code remains lightweight while sensitive operations are processed server-side.
 ...
 Backend: Hosted entirely on Vercel using Node.js Serverless Functions located in the ⁠/api/⁠ directory.
 Database: Firebase Realtime Database. Operations are executed securely via the ⁠firebase-admin⁠ SDK on the backend, preventing client-side exposure of database URLs and credentials.
 Analytics: Integrated Google Analytics 4 (GA4) with custom event tracking.
...
Integrated Libraries
 JSZip: Utilized client-side on the accounts page to bundle the JSON data fetched from ⁠api/export-data.js⁠ into a downloadable ⁠.zip⁠ archive.
 glin-profanity: Implemented within the backend API routing to evaluate user inputs against a standardized database of prohibited language.
...
The platform is configured for automated deployment via Vercel. To initialize the connection to your specific database instance (such as the ⁠bsms-rooms-default-rtdb⁠ project), you must define the following Environment Variables within your Vercel project settings:
 ⁠FIREBASE_PROJECT_ID⁠
 ⁠FIREBASE_CLIENT_EMAIL⁠
 ⁠FIREBASE_PRIVATE_KEY⁠
 ⁠FIREBASE_DATABASE_URL⁠
Because the ⁠firebase-admin⁠ SDK handles the authorization securely on the server, no configuration objects need to be exposed in the public repository.
